...
def test_api_create_project(tmp_path):
    ...
    project = create_project(name, path=str(tmp_path / name))
...
def test_api_open_project(tmp_path):
    ...
    create_project(name, path=str(project_path))
    project = open_project(name)
    assert project.name == name


def test_api_list_projects(tmp_path):
    name_one = f"one_{uuid.uuid4().hex[:8]}"
    name_two = f"two_{uuid.uuid4().hex[:8]}"
    create_project(name_one, path=str(tmp_path / name_one))
    create_project(name_two, path=str(tmp_path / name_two))
...
def test_api_open_nonexistent_project(tmp_path):
    ...
    create_project(name, path=str(tmp_path / name))
    with pytest.raises(ValueError, match="Project not found"):
        open_project(f"does_not_exist_{uuid.uuid4().hex[:8]}")
...
def test_api_create_duplicate_project(tmp_path):
    ...
    with pytest.raises(ValueError, match="already exists"):
        create_project(name, path=str(tmp_path / f"{name}_2"))


def test_api_create_with_custom_path(tmp_path):
    name = f"custom_path_project_{uuid.uuid4().hex[:8]}"
    custom_path = tmp_path / name
    project = create_project(name, path=str(custom_path))
    assert project.path == Path(custom_path)
    assert project.path.exists()
