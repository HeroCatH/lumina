...
def test_open_project(tmp_path, monkeypatch):
    monkeypatch.setenv("LUMINA_PROJECTS_ROOT", str(tmp_path))
    create_project("p1", path=str(tmp_path / "p1"))
    project = open_project("p1", path=str(tmp_path / "p1"))
    assert project.name == "p1"


def test_start_run_logs_and_finishes(tmp_path, monkeypatch):
    monkeypatch.setenv("LUMINA_PROJECTS_ROOT", str(tmp_path))
    project = create_project("p1", path=str(tmp_path / "p1"))
...
def test_start_run_default_name(tmp_path, monkeypatch):
    monkeypatch.setenv("LUMINA_PROJECTS_ROOT", str(tmp_path))
    project = create_project("p1", path=str(tmp_path / "p1"))
